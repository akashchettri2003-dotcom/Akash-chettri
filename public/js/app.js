...
function creatorStudio(){
 ...
 $("#creatorForm").onsubmit=async e=>{e.preventDefault();const b=Object.fromEntries(new FormData(e.target).entries());b.tags=b.tags.split(",").map(x=>x.trim()).filter(Boolean);try{await api("/api/creator/works",{method:"POST",body:JSON.stringify(b)});toast("Draft saved.");closeModal();loadWorks();}catch(err){toast(err.message)}}
}
