...
setInterval(() => {
  ...
  for (const key of hits.keys()) {
    const timeMark = Number(key.split(":")[1]);
    if (timeMark < nowMin - 1) hits.delete(key);
  }
}, 60000);
...
app.post("/api/progress/:id",auth,(req,res)=>{
  ...
  const chapter=Math.max(1,Number(req.body.chapter)||1);
  const position=Math.max(0,Math.min(1,Number(req.body.position)||0));
  
  db.prepare(`
    INSERT INTO progress(user_id,work_id,chapter,position) VALUES(?,?,?,?)
  ...
  `).run(req.user.id,workId,chapter,position);
